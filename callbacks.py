    ...
    async def message(self, room, event):
        ...
        for forward_name in self.config.forwards:
            if self.config.forwards[forward_name]['from'] == room.display_name:
                ...
                await send_text_to_room(self.client, self.config.forwards[forward_name]['to'], f"{self.config.forwards[forward_name]['prefix']}{ user_display_name } wrote in '{room.display_name}':<br>{event.body}", notice=True)
        logger.debug(
